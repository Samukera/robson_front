<template>
  <div>
    <h1 class="screen-reader-only">Robson, Planeje com ele</h1>
    <Modal
        v-if="modal"
        title="Escolha seu pseudônimo"
    ...
    <div v-if="!modal && !settings && !showShareModal" class="home">
      ...
      <div class="top-left">
        ...
        <PFLittleButton type="settings" popover-text="Configurações" @clicked="()=>{settings = true;}"></PFLittleButton>
        <div class="voting-on" v-if="votingOnName">
          <p class="voting-on-label">Votando para: <b>{{ votingOnName }}</b></p>
        </div>
      </div>

      <button v-if="!playerHasVoted() && !showVotes" class="button no-hover">
        <span>Escolha seu voto</span>
      </button>
      <button v-if="playerHasVoted() && !showVotes" class="button" @click="showVotesClicked()">
        <span>Mostrar votos!</span>
      </button>
      <button
      ...
      <div class="players" v-for="player in players" :key="player.id">
        ...
        <div class="name">
          <span style="color: #F0F2F2 ;">{{ player.name }}</span>
        </div>
      </div>
      ...
      <div class="results-container" v-if="showVotes && countdown === 0">
        <div class="results">
          <div class="average">Média: {{ averageValue }}</div>
            <div class="popular">Mais próximo: {{ closestValue }}</div>
        </div>
      </div>
...
const startGameMessage = computed(() => {
  if (!tickets.value || tickets.value.every(t => t.score)) {
    return 'Começar nova votação!'
  } else {
    return 'Votar na próxima!'
  }
});
